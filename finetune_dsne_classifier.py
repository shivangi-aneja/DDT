...
def train_classifier(epoch):
    ...
    for i, _ in enumerate(range(num_iter)):
        ...
        loss_src = (1 - alpha) * classification_loss(y_hat_src, y_src) + \
                   alpha * d_sne_loss(z_src=z_src, y_src=y_src, z_tgt=z_tgt, y_tgt=y_tgt)

        loss_tgt = (1 - alpha) * classification_loss(y_hat_tgt, y_tgt) + \
                   alpha * d_sne_loss(z_src=z_tgt, y_src=y_tgt, z_tgt=z_src, y_tgt=y_src)
