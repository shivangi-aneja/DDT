...
import tqdm
from common.losses.custom_losses import mmd_linear
from common.models.resnet_subset_models import EncoderLatent as Encoder1
import random

parser = argparse.ArgumentParser(description='VAE FaceForensics++ MMD FineTuning')
...
def train_classifier(epoch):
    ...
    for i, _ in enumerate(range(num_iter)):
        ...
        mmd_loss = mmd_linear(z_src, z_tgt)
        loss = classification_loss(y_hat_src, y_src) + classification_loss(y_hat_tgt, y_tgt) + alpha*mmd_loss
        loss.backward()
        train_loss = loss.item()
        optimizer.step()
        tbar.set_description(last_desc)
        logger.log(mode="train", error=train_loss / len_target_loader, epoch=epoch*i + i, n_batch=0, num_batches=1,
               scalar='avg_loss')
