...
from common.losses.custom_loss import ActivationLoss, ActivationLossFineTune
...
class Autoencoder(object):
    ...
    def __init__(self, autoencoder, model_name, recon_loss, mode, optim_kwargs, dataset, num_classes, batch_size=64,
                 ...
                 patience=30, counter=0, early_stop=False, model_dir='forensic_transfer'):
        """
        ...
        if mode == 'train':
            ...
            fine_tune_param_list = [self.autoencoder.encoder[16], self.autoencoder.encoder[20],
                                    self.autoencoder.encoder[25],
                                    self.autoencoder.decoder[0], self.autoencoder.decoder[4],
                                    self.autoencoder.decoder[8]]
        ...
        self.scheduler = StepLR(self.optim, step_size=30, gamma=0.9)
    ...
    def train_autoencoder(self, train_loader, test_loader, model_path):
        ...
        for epoch in range(self.epochs):
            ...
            rootLogger.info('[%d/%d] - ptime: %.2f Train : Total loss: %.3f, Recon loss: %.3f, Act loss: %.3f, Center loss: %.3f, acc: %.3f' % (
            (epoch + 1), self.epochs, per_epoch_ptime, avg_loss, avg_recon_loss, avg_act_loss, avg_center_loss, accuracy))
            ...
            for epoch_iter, data in enumerate(train_loader):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    if self.num_classes == 2:
                        ...
                        y_onehot.zero_()
                        y_onehot.scatter_(1, target.view(-1, 1), 1)
                        loss = alpha_recon_loss * self.recon_loss(pred_image, input_image) + alpha_act_loss * ActivationLoss(activation_vector,
                                                                                               y_onehot)
                    else:
                        # Calculate Activation Loss
                        with torch.no_grad():
                        ...
                        y_onehot_vector = torch.FloatTensor(target.size()[0], 3).cuda()
                        y_onehot_vector.zero_()
                        y_onehot_vector.scatter_(1, target.view(-1, 1), 1)
                        real_part = y_onehot_vector[:, 0]
                        fake_part = y_onehot_vector[:, 1]
                        y_onehot = torch.cat([real_part.unsqueeze(1), fake_part.unsqueeze(1), fake_part.unsqueeze(1)], dim=1)
                        # _, act_fake_index = torch.max(activation_vector[:, 1:], dim=1)
                        # act_fake_index += 1
                        # target_center_class = act_fake_index.float() * y_onehot_vector[:, 1]
                        # target_center_class = target_center_class.long()
                        loss = alpha_recon_loss * self.recon_loss(pred_image, input_image) + alpha_act_loss * ActivationLossFineTune(
                            activation_vector, y_onehot) + alpha_center_loss * self.center_loss(z_unmasked, target)
                    epoch_train_loss += loss.item()
                    epoch_recon_loss += self.recon_loss(pred_image, input_image).item()
                    epoch_act_loss += ActivationLossFineTune(activation_vector, y_onehot).item()
        ...
        with torch.no_grad():
            for epoch_iter, data in enumerate(test_loader):
                ...
                if self.num_classes == 2:
                    ...
                    latent_df = torch.mean(torch.abs(z_unmasked[:, 2 * latent_each_class:]), dim=1)
                    activation_vector = torch.stack((latent_orig, latent_f2f, latent_df), dim=1)
                    # 3 beacuse we are dealing with 3 classes here
                    y_onehot_vector = torch.FloatTensor(target.size()[0], 3).cuda()
                    y_onehot_vector.zero_()
                    y_onehot_vector.scatter_(1, target.view(-1, 1), 1)
                    real_part = y_onehot_vector[:, 0]
                    fake_part = y_onehot_vector[:, 1]
                    y_onehot = torch.cat([real_part.unsqueeze(1), fake_part.unsqueeze(1), fake_part.unsqueeze(1)],
                                         dim=1)
                    # _, act_fake_index = torch.max(activation_vector[:, 1:], dim=1)
                    # act_fake_index += 1
                    # target_center_class = act_fake_index.float() * y_onehot_vector[:, 1]
    ...
    def validate_results(self, val_loader, model_path):
        ...
        with torch.no_grad():
            for epoch_iter, data in enumerate(val_loader):
                ...
                # _, act_fake_index = torch.max(activation_vector[:, 1:], dim=1)
                # act_fake_index += 1
                # target_center_class = act_fake_index.float() * (y_onehot[:, 1] + y_onehot[:, 2])
                # target_center_class = target_center_class.long()
                loss = alpha_recon_loss * self.recon_loss(pred_image, input_image) + alpha_act_loss * ActivationLoss(activation_vector, y_onehot) \
    ...
    def validate_results_finetune(self, val_loader, model_path):
        ...
        with torch.no_grad():
            for epoch_iter, data in enumerate(val_loader):
                ...
                val_loss += loss.item()
                recon_loss += self.recon_loss(pred_image, input_image).item()
                act_loss += ActivationLossFineTune(activation_vector, y_onehot).item()
                center_loss += self.center_loss(z_unmasked, target).item()
